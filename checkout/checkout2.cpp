/**
g++ -Wall checkout.cpp conio.cpp
//tar -cvf dalton_checkout_project.tar checkout.cpp checkout2.cpp conio.h conio.cpp
//
...
void printQueue4( list<Customer>& customers, int currentMinute, conio::Color defaultTextColor );
...
int main(int argc, char* argv[], char* envp[]) {
    ...
    int simLength,               // Length of simulation (minutes)
        ...
        waitTime2 = 0,            // How long dequeued customer waited
        totalServed2 = 0,         // Total customers served
        totalWait2   = 0,         // Total waiting time
        maxWait2 = 0;
    ...
    Register1 register2 = { false, 0, -1 };
    Register2 register3 = { false, 0, -1 };     // The register gets declared and initialized
    Register2 register4 = { false, 0, -1 };
    ...
    for ( int minute=1 ; minute<=simLength ; minute++ ) {
        ...
        for ( int j=0 ; j<numArrivals ; j++ ) {
            ...
            */
            ...
            // STL version of the standard queue 'enqueue' command.
                ...
                if (customerQ2.size() < customerQ4.size()){ //
                    customerQ2.push_back(newCustomer);
                }else{
                    customerQ4.push_back(newCustomer);
                }
        }
        ...
        /*********************Simulation 2 (like a groshery store)**********************************/
           if( ! register3.hasCustomer ) { // Try to get next customer if register is has no customer.
            if ( ! customerQ2.empty() ) {

                currentCustomer1 = customerQ2.front();
                customerQ2.pop_front();
                totalServed2++;

                register3.hasCustomer = true;
                register3.remainingTime = currentCustomer1.processingTime;
                register3.customerNumber = currentCustomer1.customerNumber;
            }
        }
            if( ! register4.hasCustomer ) { // Try to get next customer if register is has no customer.
                if ( ! customerQ4.empty() ) {

                    currentCustomer2 = customerQ4.front();
                    customerQ4.pop_front();
                    totalServed2++;
...
//Sitiation 2 printout        
        cout << conio::gotoRowCol(12, 1) << "Reg3 Serving customer: ";
        if( register3.hasCustomer ) {
            cout << conio::fgColor(conio::CYAN) << register3.customerNumber << ' '
                 << conio::fgColor(defaultTextColor)
                 << conio::gotoRowCol(12, 25) << "(" << register3.remainingTime
                 << " minute" << ((register3.remainingTime == 1)? "" : "s") << " left)";
        }


        cout << conio::gotoRowCol(16, 1) << "Reg4 Serving customer: ";
        if( register4.hasCustomer ) {
            cout << conio::fgColor(conio::CYAN) << register4.customerNumber << ' '
                 << conio::fgColor(defaultTextColor)
                 << conio::gotoRowCol(16, 25) << "(" << register4.remainingTime
        ...
        cout << conio::gotoRowCol(20, 1) << "# customers finished: " << totalServed2;
        printQueue3( customerQ2, minute, defaultTextColor );
        printQueue4( customerQ4, minute, defaultTextColor );
...
/********************Simulation 2 change to false***************************/
        if( register3.hasCustomer ) {
            ...
            if( register3.remainingTime == 0 ) {
                ...
                waitTime2 = minute-currentCustomer1.arrivalTime;
        ...
        if( register4.hasCustomer ) {
            ...
            if( register4.remainingTime == 0 ) {
                ...
                waitTime2 = minute-currentCustomer2.arrivalTime;
                totalWait2 += waitTime2;
                if( waitTime2 > maxWait2 ) {
                    maxWait2 = waitTime2;
                }
     ...
     ***********************************************************************/
    cout << conio::gotoRowCol(21, 1) << endl;
    ...
    cout << conio::gotoRowCol(26, 1) << endl;
    ...
    cout<< conio::gotoRowCol(36,1)<<endl;
...
void printQueue3( list<Customer>& customers, int currentMinute, conio::Color defaultTextColor ) {
    list<Customer>::iterator it;
    cout << conio::gotoRowCol(13, 1) << "Customer queue: ";
...
void printQueue4( list<Customer>& customers, int currentMinute, conio::Color defaultTextColor ) {
    list<Customer>::iterator it;
    cout << conio::gotoRowCol(17, 1) << "Customer queue: ";
