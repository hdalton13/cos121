 ...
 * Creates human and Priest objects, infects the human, then runs simulation until time is 240.
 */
void Board::run() {
    int row, col;
    ...
    for(int pos=0; pos<numHumans; ++pos) {
        ...
        if( pos>=7 ) {
            humans[pos] = new Human(row, col, false, "Person", 0, 0, this); 
        } else {
            humans[pos] = new Priest(row, col, false, "Priest", 0, 0, this); 

        }
    }

    // Infect a random human in the range 0 to numHumans-1
    humans[random()%numHumans]->toggleInfected();

    for(currentTime=0; allInfected() == false && currentTime<240; ++currentTime) {
        // Clear screen before every new time unit
        cout << conio::clrscr() << flush;

        // Tell each human to try moving
        for(int pos=0; pos<numHumans; ++pos) {
            if( humans[pos]->getRole() != "dead" ) {
                humans[pos]->move();
            }
        }
 ...
 * For each pair of adjacent human and Priest in the simulation, blessed() makes them immuned for 
 * 5 seconds
 * For each pair of adjacent infected human and Priest, heal() makes the human healthy again
 * For each pair of adjacent deaf human and Priest, resurrect() brings the dead human back to a healthy life
 */
void Board::processInfection() {
    for( int i=0; i<numHumans; ++i ) {
        for( int j=i+1; j<numHumans; ++j ) {
            if( isNextTo(humans[i], humans[j]) ){
                if( humans[i]->getRole()=="Priest" ) {
                    // Priest functions
                    if( humans[j]->isInfected()==false ) {
                        humans[i]->bless( humans[j] );
                    } else if( humans[j]->getRole()!= "dead") {
                        humans[i]->heal( humans[j] );
                    } else {
                        humans[i]->resurrect( humans[j] );
                    }
                    // Regular human functions
                } else if( humans[i]->isInfected() && humans[j]->getBlessed() > 0 ) {
                    continue;
                } else if( humans[i]->isInfected() && humans[j]->isInfected()==false ) {
                   humans[j]->toggleInfected();
                } else if ( humans[j]->isInfected() && humans[i]->getBlessed() > 0 ) {
                    continue;
                } else if ( humans[j]->isInfected() && humans[i]->isInfected()==false ) {
                    humans[i]->toggleInfected();
                }
            }
        }
        if( humans[i]->getBlessed() > 0 ) humans[i]->setBlessed( humans[i]->getBlessed()-1 );
        if( humans[i]->isInfected() ) humans[i]->setTimeInfected( humans[i]->getTimeInfected()+1 );
        if( humans[i]->getTimeInfected() > 20 ) humans[i]->setRole("dead");
    }
